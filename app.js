...
function getVoiceSettings(){
  try{return JSON.parse(localStorage.getItem(VOICE_STORAGE_KEY))||{}}
  catch(e){return {}}
}
...
function applyUI(){
  ...
  setCssVar("--bg",u.couleur_fond);
  setCssVar("--ink",u.couleur_texte);
  setCssVar("--accent",u.couleur_principale);
  setCssVar("--footer",u.couleur_footer);
  ...
  if(u.afficher_logo_ot===false||!u.logo_ot){
    logo.style.display="none";
  }else{
    logo.style.display="block";
    logo.src=u.logo_ot+"?v="+Date.now();
    logo.alt=u.logo_alt||"Logo de l'office de tourisme";
  }
}
...
function build(){
  ...
  sitesMelanges.forEach(([id,s])=>{
    ...
    a.innerHTML=`
      <img class="photo" src="${s.photo}?v=${Date.now()}" alt="${s.titre}">
      <div class="info">
        <div class="icon" style="background:${s.couleur||"#72a45a"}">${s.icone||"📍"}</div>
        <div class="copy">
...
$("#go").onclick=()=>{
  ...
  let done=false;
  const go=()=>{if(done)return;done=true;location.href=url};
  speak(r,go);
...
function initVoiceSettings(){
  ...
  if(settingsBtn)settingsBtn.style.display=adminMode?"block":"none";
